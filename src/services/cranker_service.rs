...
pub async fn update(pool: &str, config: &CrankerConfig) -> Result<()> {
    let rpc = RpcClient::new_with_commitment(config.rpc_url.clone(), CommitmentConfig::confirmed());
    let pool = Pubkey::from_str(pool).context("Invalid pool address format")?;

    let payer = Keypair::from_base58_string(&config.payer_private_key);
    let payer: &dyn Signer = &payer;

    let stake_pool_acc = rpc
        .get_account(&pool)
        .await
        .context("Failed to fetch stake pool account")?;

    let program_id = stake_pool_acc.owner;

    let stake_pool = StakePool::deserialize(&mut stake_pool_acc.data.as_slice())
        .context("Failed to deserialize stake pool data")?;

    let validator_list_acc = rpc
        .get_account(&stake_pool.validator_list)
        .await
        .context("Failed to fetch validator list account")?;

    let ValidatorList { validators, .. } =
        ValidatorList::deserialize(&mut validator_list_acc.data.as_slice())
            .context("Failed to deserialize validator list data")?;
