...
pub async fn update_pool(
        ...
        .enumerate()
    {
        ...
        let ixs = vec![uvlb.full_ix_from_validator_slice(
            program_id,
            chunk,
    ...
    let final_ixs = vec![
        update_stake_pool_balance_ix_with_program_id(
            program_id,
            UpdateStakePoolBalance { stake_pool }.resolve_for_prog(&program_id)?,
        )?,
        cleanup_removed_validator_entries_ix_with_program_id(
            program_id,
            CleanupRemovedValidatorEntries { stake_pool }.resolve()?,
        )?,
    ];

    let final_ixs = with_auto_cb_ixs(rpc, &payer.pubkey(), final_ixs, &[], fee_limit_cb).await;

    tracing::info!("Sending final update tx");
